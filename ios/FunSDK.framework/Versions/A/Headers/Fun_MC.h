...
typedef struct SMCInitInfoV2
{
	...
	char userID[64];    // 账户ID，不传SDK内部会通过本地缓存获取
 ...
 * @return @async 消息ID：6011; param1: >=0 成功，否者失败;
 */
...
*          [无]
* 结果消息：
* 		消息 EMSG_MC_LinkDevs_Batch = 6019,  // 批量报警订阅
...
*          [无]
* 结果消息：
* 		消息 EMSG_MC_UnLinkDevs_Batch = 6020,  // 批量取消报警订阅
* 		arg1: >=0 成功，< 0错误值，详见错误码
...
int MC_GetLinkState(const char *uuid);
  ...
  * @return 结果消息 ：1.消息ID：6021 2.param1:结果，>=0成功; <0 失败，详见错误码 3.msg->Str() 设备序列号  4.pData:成功返回结果信息(JSON,需要app自己解析)，失败返回错误信息
  * @retval >=0成功; <0 失败，详见错误码
  */
 ...
 * @return 结果消息 ：1.消息ID：6022 2.param1:结果，>=0成功; <0 失败，详见错误码 3.msg->Str() 设备序列号  4.pData:成功返回结果信息(JSON,需要app自己解析)，失败返回错误信息
 * @retval >=0成功; <0 失败，详见错误码
 */
...
// 视频最多返回500条，报警消息最多返回200条，需要客户端重复查询
 ...
 * @return @async 消息ID：6023; param1: >=0 成功，否者失败;  Str()：查询过的设备序列号列表; pDdta:结果信息(Json格式)
 * @return == 0
 */
...
* @param nStartTime utc秒，默认当前时间，向前查询   *批量设备只能查询同一个时间范围
* @param nEndTime utc秒，默认当前时间，向前查询
* @param bSortType 查询类型  false:倒序(默认)，当前时间向前查询 true：顺序，从nStartTime向后查询
* @param nQueryCount 查询条数，默认500，最多500（因为设备可能分配在不同的服务器，实际查询返回的最大条数是500的成倍增加）
* @return @async 消息ID：6024; param1: >=0 成功，否者失败;  Str()：查询成功的设备序列号列表; pDdta:结果信息(Json格式，数据内容APP需重新按时间排序)
*/
 ...
 * @return @async 消息ID：6025; param1: >=0 成功，否者失败 Str:结果信息(JSON格式)
 */
 ...
 * @return @async 消息ID：6026; param1: >=0 成功，否者失败 Str:结果信息(JSON格式)
 */
 ...
 * @details 异步回调消息：id:6027 param1: >=0 成功，否者失败 Str:设备序列号 pData：消息列表信息
 */
 ...
 * @details 异步回调消息：id:6028 param1: >=0 成功，否者失败 Str:设备序列号 pData：消息列表信息
 */
 ...
 * @details 异步回调消息：id:6029 param1: >=0 成功，否者失败 Str:结果信息
 ...
 * @details 异步回调消息：id:6030 param1: >=0 成功，否者失败
 */
 ...
 * @details 异步回调消息：id:6031 param1: >=0 成功，代表数据大小，否者失败；param2:报警条数；param3：查询结束标志；Str:查询设备列表；pData:报警数据
 */
int MC_BatchDevAlarmMsgQuery(UI_HANDLE hUser, SBatchDevAlarmMsgQueryReqParams *pInfo, int nSeq = 0);

/**
 * @brief 设备报警消息查询
 * @details 支持分页&报警类型选择查询，同时兼容老的方式查询  ps:普通报警消息，分页查询无效!!!
 * @param pInfo 请求的参数信息 详见@struct SDevAlarmMsgQueryReqParams
 * @details 异步回调消息：id:6032 param1: >=0 成功，代表数据大小，否者失败；param2:报警条数；param3：查询结束标志；Str:设备序列号；pData:报警数据
 */
