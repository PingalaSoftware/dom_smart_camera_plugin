...
{
    int nDelayTime; ///< 最大延迟时间，单位ms，默认160ms
    int nEchoMode; ///< 回声消除等级 0~4； 默认 2
    int nCngMode; ///< 双方都静音的情况下，是否会填充舒适噪声 ，默认0:不填充，非0填充
    SWebRtcAecParams()
    {
        nDelayTime = 2; /**< 内部做过特殊处理、2对应的就是160ms */
        nEchoMode = 2;
        nCngMode = 0;
    }
};
