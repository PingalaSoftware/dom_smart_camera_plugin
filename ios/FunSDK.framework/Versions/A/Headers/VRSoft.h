...
typedef enum _XMVRType {
	...
	XMVR_TYPE_MULTI_LENS = 21,	// 多目变倍

	XMVR_TYPE_SINGLE_PIP = 22,	// 单张图片画中画方式

	XMVR_TYPE_MULTI_PIP = 23,	// 多目相机画中画方式

	XMVR_TYPE_TWO_LENSES_IN_ONE = 24,	// 双目变焦二合一画面
...
typedef enum _XMTwoLensesDrawMode {
	Zoomable = 0,
	FitScreen = 1,
    TopOnly = 2,
    BottomOnly = 3,

	XMTWOLENSESDRAWMODE_END
} XMTwoLensesDrawMode;

typedef enum _XMTwoLensesScreen {
	TwoLensesScreenDouble = 0,
	TwoLensesScreenLeft = 1,
	TwoLensesScreenRight = 2,

	XMTWOLENSESSCREEN_END
} XMTwoLensesScreen;
...
// 设置双目镜头显示方式
void VRSoft_SetTwoLensesDrawMode(VRHANDLE hVR, XMTwoLensesDrawMode mode);

// 设置双目镜头横向平铺方式时的左右镜头切换(其他模式无效)
void VRSoft_SetTwoLensesScreen(VRHANDLE hVR, XMTwoLensesScreen screen);

// 设置双目镜头横向平铺方式时当前是左画面还是右画面(其他模式无效)
XMTwoLensesScreen VRSoft_GetTwoLensesScreen(VRHANDLE hVR);

// 设置背景颜色
void VRSoft_SetBackground(VRHANDLE hVR, int color);
...
// 多目变倍手动切换画面（测试）
void VRSoft_Multi_SwitchLens(VRHANDLE hVR, int lensId);

// 多目变倍设置有效的目数
void VRSoft_Multi_SetLens(VRHANDLE hVR, int lensCount);

// 多目变倍获取当前有效的目数
int VRSoft_Multi_GetLensCount(VRHANDLE hVR);

// 多目变倍获取当前索引
int VRSoft_Multi_GetLensIndex(VRHANDLE hVR);
...
/**
 * VRSoft_SetRotateZoomRecover()和VRSoft_SetRotateZoom()的区别在于是否同时设置Event的参数
 * @param hVR
 * @param rotateX
 * @param rotateY
 * @param rotateZ
 * @param zoom
 */
void VRSoft_SetRotateZoomRecoverMode(VRHANDLE hVR,
						  double rotateX,
						  double rotateY,
						  double rotateZ,
						  double zoom);
...
void VRSoft_GetPTZFor180VR(VRHANDLE hVR,
		double *rotateX,
		double *rotateY,
		double *rotateZ,
		double *zoom);

void VRSoft_SetPTZFor180VR(VRHANDLE hVR,
		double rotateX,
		double rotateY,
		double rotateZ,
		double zoom);
//翻转
void VRSoft_SetRotationFor180VR(VRHANDLE hVR,
		double rotation);

// 获取当前X方向相对于[-1.0, 1.0]的偏移
float VRSoft_GetRelativeXOffset(VRHANDLE hVR);


/**
 * 设置上层缩放比例，跟setLogicZoomRange()配合使用
 * @param hVR
 * @param zoomMin
 * @param zoomMax
 * @param zoomDiv (需要切换镜头的缩放倍数分界点)
 * @param zoomValue
 */
void VRSoft_SetLogicZoom(VRHANDLE hVR,
							  double zoomMin,
							  double zoomMax,
							  double zoomDiv,
							  double zoomValue);

float VRSoft_GetLogicZoom(VRHANDLE hVR,
						 double zoomMin,
						 double zoomMax);
