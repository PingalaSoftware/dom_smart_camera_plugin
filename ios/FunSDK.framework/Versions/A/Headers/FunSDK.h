...
//---用户忘记/修改密码相关接口---
#ifndef CUSTOM_MNETSDK
//修改用户密码 支持修改非当前登录账户密码(UserName和当前登录账户名不一致时)
...
//修改用户密码--EMSG_SYS_EDIT_PWD_XM 支持修改非当前登录账户密码(UserName和当前登录账户名不一致时)
...
/** 登出接口  1.当前所有设备链接清除   2.当前登录账户信息清除 */
...
/** 当前账户绑定手机号，替换FUN_SysBindingPhone */
...
/** 获取验证码(用于当前账户绑定邮箱，替换FUN_SysSendBindingEmailCode) */
...
/** 当前账户绑定邮箱 替换FUN_SysBindingEmail */
...
// unionId:唯一ID
// szType:第三方类型(例:微信“wx”， Google“gg_xx”，Facebook“fb_xx”，line是“line_xx” xx 是username，暂时未使用，可以传任意字符串 *区分大小写)
...
/*******************用户服务相关的接口**************************
* 方法名: 按字段获取设备信息
* 描  述: 获取设备信息通用接口，后面增加新字段的话，也不要再更改SDK了。。
*         IOS使用此接口，Android使用JNI层提供的接口，参数不一样。。
* 返回值:
*      [返回的数据长度]
* 参  数:
*      输入(in)
*          [szDevId:设备序列号 或 ip+port]
*          [szField:json字段，例如"createTime"]
*      输出(out)
*          [szValue：信息]
* 结果消息：
* 		消息id:EMSG_SYS_GET_LOGIN_ACCOUNT_CODE 5076
...
/*******************用户服务相关的接口**************************
* 方法名: 修改缓存设备信息
* 描  述: 修改缓存设备信息，不会影响服务器端存储的设备信息，下次获取设备列表的时候修改的缓存信息作废。
*		  为了兼容客户端添加，删除设备时，未重新获取设备列表刷新数据。
* 返回值:
*      [>=0成功，否者失败]
* 参  数:
*      输入(in)
*          [szDevId:设备序列号 或 ip+port]
*          [nSystemTime:设备添加的系统时间]
*          [nNumberOfSharedAccounts:分享账户数量]
*          [nOptionType:操作类型  0：添加 1：修改 2：删除 对应枚举 E_SYS_MODIFY_DEV_INFO]
*          [szExJson:nOptionType == 0 || nOptionType == 1时生效，如果需要添加，修改其他的字段信息，通过此json传递，
 ...
 * @return @async 消息ID：5094; param1: >=0 成功，否者失败 Str:结果信息(JSON格式)
 */
...
/*******************设备状态更新相关接口**************************
* 方法名: 分类型获取设备状态
* 描  述: 分类型获取设备状态（去服务器查询）
* 返回值:
*      [无]
* 参  数:
*      输入(in)
*          [sDevIds:设备序列号/ip 多个设备使用";"分隔]
*          [nQueryType:查询类型，详见枚举EDevStatusType]
*      输出(out)
*          [无]
* 结果消息：
* 		消息id:EMSG_SYS_GET_DEV_STATE = 5009
* 		查询结果：每一个设备单独返回
* 		parame1: >=0 成功，< 0错误值，详见错误码
* 		Str():设备序列号
****************************************************/
...
* 描  述: 获取第三方登陆uname upass，然后与用户进行绑定, 用户名密码需上层传递，和SDK内部缓存的当前登录用户名密码无关联
* 返回值:
*      [无]
* 参  数:
*      输入(in)
 ...
 * @details 异步回调消息：id:5206 param1: >=0 成功，否者失败, Str:服务器返回结果信息
 */
...
/*******************用户服务相关的接口**************************
 * 方法名: 第三方登录报警订阅监听开启/关闭/状态查询
 * 描  述: 第三方登录报警订阅/取消订阅/订阅信息查询
 * 返回值:
 *      [无]
 * 参  数:
 *      输入(in)
 *          [szDeviceSN:设备序列号，不支持ip：port]
 *          [sThirdPartyType:第三方登录类型，例如"line"/"wx"等]
 *      输出(out)
 *          [无]
 * 结果消息：
 *         消息id:    EMSG_SYS_THIRD_PARTY_ALARM_LISTEN_OPEN = 5083,         // 开启第三方报警监听
 ...
 * @details 异步回调消息：id:5203 param1: >=0 成功，否者失败, Str:服务器返回结果信息
 */
 ...
 * @details 异步回调消息：id:5204 param1: >=0 成功，否者失败, Str:服务器返回结果信息
 */
...
/*******************caps服务相关的接口**************************
 * 方法名: 从服务器获取设备的能力集
 * 描  述: 从服务器获取设备的能力集,返回服务器下发的整个json对象给请求端
 * 注意:  为了兼容旧版本，新增的接口，后续APP兼容后完全替代Fun_SysIsDevMasterAccountFromServer接口
 * 返回值:
 *      >=0成功，否者失败，详见错误码
 * 参  数:
 *      输入(in)
 *          [szDevSysInfo:请求的整个json字段(请求端自行组装)]
 *      输出(out)
 *          [无]
 * 结果消息：
 *         消息id:  EMSG_SYS_GET_DEV_CAPABILITY_SET = 5089, // 获取设备能力集  用来替代EMSG_SYS_GET_ABILITY_SET
 *         param1: >=0 成功，<0 错误值，详见错误码
 *         Str：服务器返回的结果信息
 *         pData: 设备序列号
 ****************************************************/
 ...
 * @return @async 消息ID：5092; param1: >=0 成功，否者失败 Str:结果信息(JSON格式)
 * @return >=0 成功，<0 错误值，详见错误码
 */
...
/**
 * @brief 发送链接邮件用于注册用户
 * @details 异步回调消息：id:5097 param1: >=0 成功，否者失败, Str:UserId信息
 * @param szEmail 邮箱
...
/**
 * @brief 检测用户是否已激活
 * @details 异步回调消息：id:5099 param1: >=0 成功, -604056表示未激活
 * @param szUserID 用户ID
 */
...
/**
 * @brief 发送链接邮件用于重置密码
 * @details 异步回调消息：id:5201 param1: >=0 成功，否者失败, Str: 结果信息
 * @param szEmail 邮箱
 */
 ...
 * @details 异步回调消息：id:5202 param1: >=0 成功, -604027表示用户找回密码失败 -604065表示用户未找回密码，app端应继续监听；-6044028表示用户找回密码链接过期
 * @param szUserID 用户ID
 */
...
/**
 * @brief 影子服务器获取设备配置
 * @deprecated 建议使用Fun_GetDevCfgsFromShadowService
 * @details 接口支持单个/多个配置同时获取，SDK内部会缓存到内存当中，重启APP则清空。   // Dev--->CfgName  CfgName--->CfgJson
 * @details 异步回调消息：id:5087 param1: >=0 成功，否者失败, Str:设备序列号 pData：服务器返回结果信息
...
/**
 * @brief 影子服务设备配置状态监听
 * @deprecated 建议使用Fun_AddShadowServiceListener
 * @details 定时查询功能，查询结果有变化通知上层，接口支持单个/多个配置同时监听
 * @details 异步回调消息：id:5088 param1: >=0 成功，否者失败, Str:设备序列号 pData：配置状态变化结果，json格式与查询接口FUN_SysGetCfgsFormShadowServer一致
 * @param sDeviceSN 设备序列号，不支持ip+port方式
...
/**
 * @brief 移除影子服务设备配置监听
 * @deprecated 建议使用Fun_RemoveShadowServiceListener
 */
 ...
 * @brief 影子服务器获取设备配置
 * @details 接口支持单个/多个配置同时获取，SDK内部会缓存到内存当中，重启APP则清空。   // Dev--->CfgName  CfgName--->CfgJson
 * @details 异步回调消息：id:8800 param1: >=0 成功，否者失败, Str:设备序列号 pData：服务器返回结果信息
...
/**
 * @brief 设置设备离线配置到影子服务
 * @details 异步回调消息：id:8801 param1: >=0 成功，否者失败, Str:设备序列号 pData：结果信息
 * @param szDeviceSN 设备序列号，不支持ip+port方式
 ...
 * @details 异步回调消息：开启监听结果：id:8803 param1: >=0 成功，否者失败, Str:设备序列号 pData：结果信息
 * @details 异步回调消息：配置状态变化结果通知： id:8802 param1: >=0 成功，否者失败, Str:设备序列号 pData：配置状态变化结果，json格式与查询接口Fun_GetDevCfgsFromShadowService格式一致
 ...
 * @brief 移除影子服务设备配置监听
 * @param szDeviceSNs 设备序列号，多个以;分隔，不支持ip+port方式
 * @details 异步回调消息：停止监听结果：id:8804 param1: >=0 成功，否者失败, Str:设备序列号 pData：结果信息
 */
 ...
 * @return @async 消息ID：5150; param1: >=0 成功，否者失败 param2:通道号 param3:配置ID pDdta:结果信息
 * @return >=0消息发送成功，否者失败
 */
...
typedef enum EFUN_ERROR
{
	...
	// HTTP协议通用错误
	EE_HTTP_INVALIDREQ = 500400, 			    // 400 无效请求
	EE_HTTP_SERVER_UNAVAIL = 500503, 		    // 503 服务器目前无法使用（由于超载或停机维护）。通常，这只是暂时状态
