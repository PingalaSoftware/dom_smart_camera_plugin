...
@implementation DomCameraPlugin {
    ...
    NSString *flutterResultMethod;
    
    //    NSMutableDictionary<NSString *, FlutterResult> *methodResults;
...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    if ([@"WIFI_PERMISSION" isEqualToString:call.method]) {
        ...
        flutterResultMethod = call.method;
        ...
        [self.customViewFactory snapImage];
        result(@[@1]);
        ...
        NSDictionary *arguments = call.arguments;
        NSInteger position = [arguments[@"position"] integerValue];
        NSLog(@"PLAY_FROM_POSITION %ld", position);
        ...
        if (position < [fileArrayPlayback count]) {
            ...
            [self.pbViewFactory startPlayBack:self.msgHandle devId:mainCameraId];
...
//    }
    ...
    switch (msg->id) {
        case EMSG_STOP_PLAY:{
            if (flutterResult && [@"STOP_STREAM" isEqualToString:flutterResultMethod]) {
                flutterResult(@[@1]);
                flutterResult = nil;
            }
        }
        ...
        case EMSG_ON_PLAY_BUFFER_BEGIN: {
            if (flutterResult && ![@"PLAYBACK_LIST" isEqualToString:flutterResultMethod]) {
                flutterResult(@[@1]);
                flutterResult = nil;
            }
        }
        ...
        case EMSG_DEV_GET_CONFIG_JSON:
            if(isSetRecordConfig) {
                ...
                NSLog(@"for wifi info got: %@", dicInfo);
                // NEED TO COMPLETE
                if (flutterResult) {
                    flutterResult(@[@false, @"Unable to get WiFi details"]);
                    flutterResult = nil;
                }
            }
            else
