...
public class DeviceClass {
  ...
  static void getCameraName(String cameraId, myDomResultInterface resultCb) {
    ...
    resultCb.onSuccess(new ArrayList<>(Collections.singleton(xmDevInfo.getDevName())));
  }

  static void setCameraName(String cameraId, String newName, myDomResultInterface resultCb) {
    manager.modifyDevName(cameraId, newName, new BaseAccountManager.OnAccountManagerListener() {
      public void onSuccess(int msgId) {resultCb.onSuccess(new ArrayList<>());}
      public void onFailed(int msgId, int errorId) {resultCb.onFailed("" + errorId, "Set name failed ["+errorId+"]");}
      public void onFunSDKResult(Message msg, MsgContent ex) {}
    });
  }

  static void cameraLoginState(String cameraId, DeviceClass.myDomResultInterface result) {
  ...
  static void getConfig(String cameraId, String type, myDomResultInterface resultCb) {
    DevConfigManager devConfigManager = DeviceManager
            .getInstance()
            .getDevConfigManager(cameraId);
    ...
    DevConfigInfo devConfigInfo = DevConfigInfo.create(
            new DeviceManager.OnDevManagerListener() {
              public void onSuccess(String devId, int operationType, Object result) {
                System.out.println("S: devId: "+ devId);
                System.out.println("S: operationType: "+ operationType);
                System.out.println("S: result: "+ result);

                String currentDevConfig;
                if (result instanceof String) currentDevConfig = (String) result;
                else currentDevConfig = JSON.toJSONString(result);
                ArrayList dataList = new ArrayList<>();
              ...
              public void onFailed(String devId, int msgId, String jsonName, int errorId) {
                System.out.println("S: devId: "+ devId);
                System.out.println("S: msgId: "+ msgId);
                System.out.println("S: errorId: "+ errorId);
                System.out.println("S: jsonName: "+ jsonName);
                if(errorId == -11406) {
                  resultCb.onFailed("" + errorId, "The configuration device does not support or is not a configuration type");
                } else {
    ...
    System.out.println("get config called with: "+ type);
  ...
  static void updateConfig(String cameraId, String type, String newData, myDomResultInterface resultCb) {
    DevConfigManager devConfigManager = DeviceManager.getInstance().getDevConfigManager(cameraId);
    DevConfigInfo devConfigInfo = DevConfigInfo.create(
            new DeviceManager.OnDevManagerListener() {
              public void onSuccess(String devId, int operationType, Object result) {
                resultCb.onSuccess(new ArrayList<>());
  ...
  static void liveStream(Context context, String cameraId, ViewGroup view, myDomResultInterface resultCb) {
    monitorManager = DeviceManager.getInstance().createMonitorPlayer(view, cameraId);
    monitorManager.startMonitor();
    monitorManager.setChnId(1);
    monitorManager.setOnAudioDecibelListener(new OnAudioDecibelListener() {public void onVolume(double v) {}});
  ...
  public static void capture(Context context) {
    ...
    File domFolder = new File(Environment.getExternalStorageDirectory() + File.separator +
              Environment.DIRECTORY_DCIM + File.separator + "DOM");
  ...
  private void getDevWiFiSignalLevel(String cameraId) {
        DevConfigManager devConfigManager = DeviceManager
      .getInstance()
      .getDevConfigManager(cameraId);
      ...
      public void onSuccess(String s, int i, String jsonData) {
        if (jsonData != null) {
          ...
          if (handleConfigData.getDataObj(jsonData, WifiRouteInfo.class)) {
            ...
            System.out.println("s: wifiRouteInfo: "+ wifiRouteInfo);
          }
        }
      }
